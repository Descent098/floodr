...
import RuntimeCalc from "../../../components/runtime-calc.svelte"
...
<details>
  ...
  No, not explicitly. The amount of time a benchmark runs for is a product of all of the other parameters. Below is a calculator to help work it out:

<Card>
<RuntimeCalc client:load/>
</Card>
  
  
  You can approximately achieve this using the following formula:


<div style="font-size-adjust: .75;">total time $$\approx \frac{z*i}{c-i}$$</div>
...
- d = delay between request (if a delay action(s) exists)
  - Can also set in benchmark [directly](/floodr/benchmark-reference/actions/delay)
- i = number of iterations i$$\geq 1$$
- s = Delay at the start of each iteration
  - Can be calculated if using a `rampup` value using $$d_{iteration} = \frac{rampup}{i}$$
...
- c = concurrency, or the number of threads the benchmark makes available c$$\leq$$i
